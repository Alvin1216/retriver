...
def download_data_from_pubmed(keyword='fever', numbers=15, filename='example'):
    ...
    query_artical_abstract_content = {'db': 'pubmed',
                                      'id': str(id_list),
                                      'retmode': 'xml'}
    abstract_string = requests.post(query_artical_abstract_url, data=query_artical_abstract_content).text
    # parse_xml_abstract_title(abstract_string)
    # print(abstract_string)
    return abstract_string


def parse_xml_abstract_title(abstract_string):
    ...
    for i in range(0, len(abstract_root.findall('./PubmedArticle'))):
        ...
        if (len(abstract_root.findall('./PubmedArticle')[i].findall('./MedlineCitation/Article/Abstract')) == 0):
            # print("not find abstract~")
            count = count + 1
            # print('\n')
        else:
            # print("find abstract~")
            article_title.append(
                abstract_root.findall('./PubmedArticle')[i].findall('./MedlineCitation/Article/ArticleTitle')[0].text)
            ...
            if (len(abstract_text_element) > 1):
                ...
                article_content.append(abstract_text_element[0].text)
...
def load_from_file():
    ...
    title, content = parse_xml_abstract_title(xml)
...
def load_from_api():
    keyword = input("請輸入想找的關鍵字:")
    xml = download_data_from_pubmed(keyword)
    # xml=load_pubmed_from_file(path)
    title, content = parse_xml_abstract_title(xml)
